...
def preprocess_tensor_images(images, mode="crop", image_size=518):
    """
    A function to preprocess tensor images for model input.
    This assumes the images should have the same shape for easier batching, but our model can also work well with different shapes.

    Args:
        images (torch.Tensor): Input tensor of images with shape (S, 3, H, W) where S is the number of images
        mode (str, optional): Preprocessing mode, either "crop" or "pad".
                             - "crop" (default): Sets width to 518px and center crops height if needed.
                             - "pad": Preserves all pixels by making the largest dimension 518px
                               and padding the smaller dimension to reach a square shape.
        image_size (int, optional): Target image size. Defaults to 518.

    Returns:
        torch.Tensor: Batched tensor of preprocessed images with shape (S, 3, H, W)

    Raises:
        ValueError: If the input tensor is empty or if mode is invalid
    ...
    # Ensure input is 4D tensor (S, 3, H, W)
    if images.dim() == 3:
        images = images.unsqueeze(0)
    elif images.dim() != 4:
        raise ValueError("Input tensor must have shape (S, 3, H, W) or (3, H, W)")
    ...
    processed_images = torch.stack(processed_images)  # concatenate images

    return processed_images


def load_and_preprocess_images(image_path_list, mode="crop", image_size=518):
    """
