...
def unproject_depth_map_to_point_map_torch(
    ...
    """
    使用 PyTorch 将一批深度图反投影到三维世界坐标。
    这个版本是可微分的，可以用于端到端训练。

    Args:
        depth_map (torch.Tensor): 形状为 [B, S, H, W] 或 [B, S, H, W, 1] 的深度图
        extrinsics_cam (torch.Tensor): 形状为 [B, S, 3, 4] 的相机外参
        intrinsics_cam (torch.Tensor): 形状为 [B, S, 3, 3] 的相机内参

    Returns:
        torch.Tensor: 形状为 [B, S, H, W, 3] 的三维世界坐标
    """
    # 确保所有输入都在同一个设备上
    ...
    # 创建像素坐标网格
    # torch.meshgrid 接受一维张量，所以我们用 arange
    # indexing='ij' 使得输出的 y_cam 在第一个维度，x_cam 在第二个维度，匹配 (H, W)
    y_cam, x_cam = torch.meshgrid(
        torch.arange(H, device=device, dtype=torch.float32),
        torch.arange(W, device=device, dtype=torch.float32),
        indexing='ij'
    )
    # 将网格扩展到与输入匹配的维度 [B, S, H, W]
    # 使用 unsqueeze 和 expand 避免大量复制数据
    x_cam = x_cam.unsqueeze(0).unsqueeze(0).expand(B, S, H, W)
    y_cam = y_cam.unsqueeze(0).unsqueeze(0).expand(B, S, H, W)
    
    # 确保深度图是 [B, S, H, W]
    depth_flat = depth_map.squeeze(-1)

    # --- 核心反投影计算 ---
    # 1. 像素坐标 -> 相机坐标系下的齐次坐标
    # [B, S, 3, 3] -> [B, S, 1, 1, 3, 3] for broadcasting
    # [B, S, H, W] -> [B, S, H, W, 1]
    # 结果: [B, S, H, W, 3]
    fx = intrinsics_cam[:, :, 0, 0].view(B, S, 1, 1)
    fy = intrinsics_cam[:, :, 1, 1].view(B, S, 1, 1)
    cx = intrinsics_cam[:, :, 0, 2].view(B, S, 1, 1)
    cy = intrinsics_cam[:, :, 1, 2].view(B, S, 1, 1)
    
    x_lift = (x_cam - cx) / fx * depth_flat
    y_lift = (y_cam - cy) / fy * depth_flat
    z_lift = depth_flat

    # 合并为相机坐标系下的点 [B, S, H, W, 3]
    cam_points = torch.stack([x_lift, y_lift, z_lift], dim=-1)

    # 2. 相机坐标 -> 世界坐标
    # 将 cam_points 转换为齐次坐标 [B, S, H, W, 4]
    ones = torch.ones_like(cam_points[..., :1])
    cam_points_hom = torch.cat([cam_points, ones], dim=-1)

    # 计算世界到相机的变换矩阵的逆，即相机到世界的变换
    # R_inv = R.T, t_inv = -R.T @ t
    R = extrinsics_cam[..., :3, :3] # [B, S, 3, 3]
    t = extrinsics_cam[..., :3, 3:] # [B, S, 3, 1]
    
    R_inv = R.transpose(-1, -2) # [B, S, 3, 3]
    t_inv = -torch.bmm(R.transpose(-1, -2).reshape(B*S, 3, 3), t.reshape(B*S, 3, 1)).reshape(B, S, 3, 1)

    # 构造相机到世界的变换矩阵
    cam_to_world = torch.cat([R_inv, t_inv], dim=-1) # [B, S, 3, 4]

    # 应用变换
    # cam_points_hom: [B, S, H, W, 4] -> [B, S, H*W, 4]
    # cam_to_world: [B, S, 3, 4] -> [B, S, 4, 3] after transpose
    world_points = cam_points_hom.view(B, S, H*W, 4) @ cam_to_world.transpose(-1, -2)
    world_points = world_points.view(B, S, H, W, 3)
